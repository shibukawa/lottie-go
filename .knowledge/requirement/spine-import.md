...
mapping:
  ...
  keys: sampled every frame, then reduced greedily — from each key the next is pushed as far as every sample between is reproduced by the two keys: each must sit within Tolerance of the straight line joining them (a path's vertices interpolate along straight lines), and a cubic-bezier easing least-squares-fitted to how far along that line each sample sits (Lottie's per-key o/i) may then place it TimingTolerance early or late; hold frames (an attachment switch) and the frame after are always keys; color and opacity keep an exact, linear tolerance. Eased Spine motion is never collinear, so without a tolerance every frame is a key (user request 2026-09-03: 1 px; easing with a looser timing budget, 3 px, on the reasoning that once the poses are right, reaching them a little early or late is not a visible defect)
  keys_measured: spineboy hull — every frame 9,378; 1 px linear 5,731; 1 px + easing 3 px 5,058; 2 px + easing 4 px 3,848. The spatial budget dominates: bone rotation moves vertices along arcs, which no timing curve straightens
...
tool: "lottierepack -import-spine hero.json -dir work [-out hero.lottie] [-atlas f] [-images dir] [-skin a,b] [-fps 30] [-scale 1] [-mesh hull|triangles] [-tolerance 1] [-timing-tolerance 3] [-bounds union|skeleton] [-bones] [-machine=false]"
open:
