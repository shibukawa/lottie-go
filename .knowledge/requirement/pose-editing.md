...
Status: implemented — editor/pose.go, editor/poseui.go, the Pose pane in
editor/inspector.go. Adjust a rig's pose at a selected keyframe by dragging the
part on stage, instead of editing editor/genpresets Go source or raw JSON and
...
promotion: editing a property that is static across the clip materializes a key at every existing pose time holding the old value, then writes the new value at the selected time — the inverse of editor/genpresets track()
...
deviations:
  ...
  - the pane names the editor/genpresets joint beside the layer (upper-arm-near -> arms(near)), which is the transcription aid the probe role asks for
...
role:
  decided: for testdata/presets the editor is a probe, not the source of truth. editor/genpresets keeps authoring those clips; a pose found by dragging is transcribed back into its clips.go pose values by hand
