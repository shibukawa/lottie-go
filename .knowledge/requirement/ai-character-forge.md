...
tooling:  # cmd/lottieforge — one command, four subcommands; designed, not built
  ...
  rig: parts + base preset -> bundle; default writes t1 (concept:uv-morph-rig — trace, simplify, fit into slot space, identity UV, rewrite every clip's image layers as shape layers, emit the texture docs); -raster writes t0
  morph: bakes the spec's morph list into path keys of the named clips; vertex counts preserved; re-runnable, since generators start from the stored rest contour
...
rules:
  ...
  - contours are star-shaped about their centroid or the part is split (concept:texture-uv-pipeline mesh_vertex limits) — hair, capes and tails are their own slots
...
open:
  - whether hair, cape and tail slots get a default follow morph derived from the parent's rotation track, or stay static until asked
  ...
  - ear clipping in the renderer would lift the star-shaped rule and let hair be one slot
```
